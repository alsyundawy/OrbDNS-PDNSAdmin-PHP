...
final class ZonePermission
{
    ...
    {
        $sql = 'SELECT can_read FROM zone_permissions WHERE user_id = ? AND zone_id = ? LIMIT 1';
        $stmt = Database::query($sql, [$userId, $zoneId]);
        $row = $stmt->fetch();
    ...
    {
        $sql = 'SELECT can_write FROM zone_permissions WHERE user_id = ? AND zone_id = ? LIMIT 1';
        $stmt = Database::query($sql, [$userId, $zoneId]);
        $row = $stmt->fetch();
